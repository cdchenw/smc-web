...
import { ListItem, IDropdownSettings } from "./multi-select-dropdown.model";
    ...
    if (!value) {
      ...
      this._data = value.map((item: any) =>
        typeof item === "string" || typeof item === "number"
          ? new ListItem(item)
          : new ListItem({
              id: item[self._settings.idField],
              text: item[self._settings.textField],
              isDisabled: item[self._settings.disabledField]
            })
      );
    }
  }
